...
bool find_path(int start, int end, int parents[], int m, int array[100][100], int numberOfRows, int numberOfColumns, FILE *file)
{
	...
	int startCoordX=0, startCooordY=0;
	...
	/*for (int i = 0; i <numberOfRows; i++ )
	{
		...
		{
			if (start == mapFunc(i, j, numberOfColumns))
			{
				startCoordX = i;
				startCooordY = j;
			}

			else if (end == mapFunc(i, j, numberOfColumns))
			{
				endCoordX = i;
				endCoordY = j;
			}
	...
	startCoordX = find_i(numberOfColumns, start);
	startCooordY = find_j(numberOfColumns, start, startCoordX);
	...
	if (end == -1) {
		...
		//fprintf(file, "There is no path from  (%d, %d) to (%d, %d)\n", startCoordX, startCooordY, endCoordX, endCoordY);
	}
	else if ((start == end)) {
		if (debug) printf("\n%d", start);
		...
		is_path = find_path(start, parents[end], parents, m, array,numberOfRows,numberOfColumns,file);
		if (debug)

			convertVertexToCell(m, array, end);
		printf(" %d", end);
	}
...
bool find_path(graph *g, int start, int end, int m, int array[100][100],int numRows, int numColumns, FILE *out)
{
	...
	if ((start < 1) || (start > g->nvertices)) {
		...
		initialize_search(g);
		bfs(g, start);
		is_path = find_path(start, end, parent, m, array,numRows,numColumns, out); // now call the version of find_path that has the parent array as an argument
		if (debug) printf("\n");
	}
	return(is_path);
}
