...
BINARY_TREE_TYPE *insert(ELEMENT_TYPE e,  BINARY_TREE_TYPE *tree ) {
   ...
   /*else if (e.frequency < (*tree)->element.frequency) { /* assume the number field is the key /
      insert(e, &((*tree)->left));
   }
...
int filePrint(FILE*fp_out,BINARY_TREE_TYPE tree, int n) {

	//int i;

	if (tree != NULL) {
		filePrint(fp_out,tree->right, n + 1);

		//for (i = 0; i<n; i++) printf("        ");
		//fprintf( "%-10s %-5d (%d)\n", tree->element.string, tree->element.frequency,n);
		fprintf(fp_out, "%-13s%d (%d)\n", tree->element.string, tree->element.frequency, n);
		// printf("%-10s \n", tree->element.string);

		filePrint(fp_out,tree->left, n + 1);
	}
	return(0);
}
...
/*void height_of_tree(){
printf("Tree height: unknown at present\n");
}
void average_number_of_probes(){
printf("Average number of probes: unknown at present\n");*/
